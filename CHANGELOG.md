...
- **App Store JWS certificate chain validation against a pinned Apple root.** Redeem validates the
  App Store JWS certificate chain against a pinned Apple root, with marker OIDs and validity
  windows. Chain validation is implemented directly in `src/x509.ts`, since Workers WebCrypto offers
  none; runtime dependencies remain at zero. (#71, fleet-chezmoi#1609)

### Fixed

- **base64url decoding** of inputs containing an underscore. (#71)
- **Metered spend allocation.** Allocates metered spend across the allowance and credit pools
  whenever a request is priced, rather than only while the balance still reads allow; attaches
  request metadata at the STT gateway call site so reconciliation can join on it; ensures a
  finalized STT session records its usage. `tests/money-path.test.ts` states the invariant that for
  any metered row `from_allowance` plus `from_credit` equals `micro_usd`, and holds both metering
  doors to it. (#72, fleet-chezmoi#1610)
- **STT error responses carry a stable client message.** Provider detail is logged server-side and
  is not part of the response; clients branch on `code`, not on message prose. (#69)
